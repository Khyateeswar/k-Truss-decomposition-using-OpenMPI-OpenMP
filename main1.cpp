...
// correct
void prefil(unordered_map<int,unordered_map<int,int>> &adj,int n,int k){// n is number of nodes and k is k-truss
    vector<int> del;
    for(int i=0;i<n;i++){
        if(adj[i].size()<k-1){
            del.push_back(i);
...
int main(int argc, char** argv){
    ...
    for(int i=1;i<tot;i++){
        ...
        if(f=="--i"){
            ...
            //cout<<"no need of p and taskid in our assignment"<<endl;
        }    
    }
